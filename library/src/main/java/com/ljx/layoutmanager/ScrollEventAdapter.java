...
import androidx.recyclerview.widget.GridPageLayoutManager.LayoutParams;
...
public final class ScrollEventAdapter extends RecyclerView.OnScrollListener {
    ...
    private final @NonNull GridPageLayoutManager mLayoutManager;
    ...
    public ScrollEventAdapter(@NonNull GridPageLayoutManager layoutManager) {
    ...
    public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
        ...
        if (mDispatchSelected) {
            ...
            mDispatchSelected = false;
            boolean scrollingForward = dy > 0 || (dy == 0 && dx < 0 == mLayoutManager.isLayoutRTL());
    ...
    private void updateScrollEventValues() {
        ...
        int sizePx = mLayoutManager.getTotalSpace();
        int offset = mLayoutManager.getPagerStartOffset(snapView);
        boolean shouldReverseLayout = mLayoutManager.shouldReverseLayout();
        int start = shouldReverseLayout ? sizePx - offset : offset;
        LayoutParams layoutParams = (LayoutParams) snapView.getLayoutParams();
        int pageIndex = layoutParams.getPageIndex();
        values.mOffsetPx = start == 0 ? 0 : sizePx - start;
        values.mPosition = start == 0 ? pageIndex : pageIndex - 1;
     ...
     * {@link androidx.viewpager2.widget.ViewPager2#endFakeDrag()} is called.
     */
     ...
     * the ViewPager is settling towards its {@link androidx.viewpager2.widget.ViewPager2#getCurrentItem() current item}, or
    ...
    public ScrollEventValues getScrollValues() {
        updateScrollEventValues();
        return mScrollValues;
    }
